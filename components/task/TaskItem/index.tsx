      ...
      <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4">
        ...
        <div className="flex-1 min-w-0 order-2 md:order-1">
          {isEditing ? (
            <EditMode
              ...
              onSave={() => { onEdit(task.id, { title: editTitle, description: editDesc }); setIsEditing(false); }}
              onCancel={() => { setEditTitle(task.title); setEditDesc(task.description || ""); setIsEditing(false); }}
            />
          ) : (
            <ViewMode
              task={task}
        ...
        <div className="flex md:flex-col items-end gap-4 shrink-0 order-1 md:order-2">
          ...
          <div className="flex gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
            {permissions.canEdit && (
              <button onClick={() => setIsEditing(true)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition">
                <Edit2 className="w-4 h-4" />
              </button>
            )}
            {permissions.canDelete ? (
              <button onClick={() => confirm("Delete?") && onDelete(task.id)} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition">
                <Trash2 className="w-4 h-4" />
              </button>
            ) : (
              <div className="p-2 text-gray-300 cursor-not-allowed"><Lock className="w-4 h-4" /></div>
            )}
          </div>
        </div>
      </div>
